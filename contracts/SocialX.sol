...
contract SocialX {
    ...
    function create(uint256 app_id, bytes calldata sign_data, uint256 node_id, string calldata title, string calldata content) external predict(sign_data) {
        emit Create(app_id, ++apps[app_id].topics);
    }

    function append(uint256 app_id, bytes calldata sign_data, uint256 target_location_id, string calldata topic_hash, string calldata content) external predict(sign_data) {
        emit Append(app_id, ++apps[app_id].appends);
    }
    function reply(uint256 app_id, bytes calldata sign_data, uint256 target_location_id, string calldata topic_hash, string calldata content) external predict(sign_data) {
        emit Reply(app_id, ++apps[app_id].repays);
    }
    function reply(uint256 app_id, bytes calldata sign_data, uint256 target_location_id, string calldata topic_hash, string calldata content, uint256 target_reply_location_id, string calldata reply_hash) external predict(sign_data) {
        emit Reply(app_id, ++apps[app_id].repays);
    }
    function tags(uint256 app_id, bytes calldata sign_data, bytes[] calldata data) external predict(sign_data) {
        emit Tags(app_id);
    }
    function follow(uint256 app_id, bytes calldata sign_data, address target_address, bool follow_status, string calldata remark) external predict(sign_data) {
        emit Follow(app_id);
    }
