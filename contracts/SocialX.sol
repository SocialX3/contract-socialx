...
contract SocialX {
    ...
    uint256 public immutable NETWORK_ID;
    ...
    constructor(uint256 network_id) {
    ...
    function create(uint256 app_id, bytes calldata data, uint256 node_id, string calldata title, string calldata content, bytes[] calldata tags) external predict(data) {
        emit Create(app_id, ++apps[app_id].topics);
    }

    function append(uint256 app_id, bytes calldata data, uint256 target_network_id, string calldata topic_hash, string calldata content) external predict(data) {
        emit Append(app_id, ++apps[app_id].appends);
    }
    function reply(uint256 app_id, bytes calldata data, uint256 target_network_id, string calldata topic_hash, string calldata content) external predict(data) {
        emit Reply(app_id, ++apps[app_id].repays);
    }
    function reply(uint256 app_id, bytes calldata data, uint256 target_network_id, string calldata topic_hash, string calldata content, uint256 reply_network_id, string calldata reply_hash) external predict(data) {
        emit Reply(app_id, ++apps[app_id].repays);
    }
